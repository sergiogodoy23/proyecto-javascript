let containerPastEvents = document.getElementById("container-past-events")
...
function filters(array) {
  ...
  const filtros = Array.from(checkboxes).map((checkbox) => checkbox.id);
  
    const busqueda = inputSearch.value.toLowerCase().trim();
  
    const elementsFilters = array.filter((event) => {
      if (filtros.length === 0) return true;
      return event.category
      .split(',')
      .some((opcion) => filtros.includes(opcion.trim()));
    });
  
    const elementsFiltersSearch = elementsFilters.filter((elemento) =>
      elemento.name.toLowerCase().includes(busqueda)
    );

    if(elementsFiltersSearch.length === 0){
    ...
    renderEvents(elementsFiltersSearch);
  }
...
const renderCategories = (array) => {
 ...
 const renderEvents = (array) => {
    ...
    const eventsFilters = array.filter(event => event.assistance)
    ...
    eventsFilters.sort((a,b)=> new Date(a.date) - new Date(b.date))
     ...
     eventsFilters.forEach((event) => {
       ...
       containerPastEvents.innerHTML = result 
 }
